...
searchButton.on("click", function (event) {
    ...
    $.ajax({
        ...
        var UVRating = "https://api.openweathermap.org/data/2.5/uvi?lat=" + lat + "&lon=" + lon + "&appid=a0eab1d4c4a0b9a91854301eece7ccae"
        $.ajax({
            url: UVRating,
            method: "GET"
        }).then(function (response) {
        ...
        $.ajax({
            ...
            for (var i = 0; i < response.list.length; i++) {
                ...
                if (hour.hour() == 12) {
                    let date = $("<h3>");
                    date.text(moment(response.list[i].dt_txt).format('MMM Do YYYY'));
                    $(box).append(date);
